...
```text
In the problem there are a giraffe eating tree. the problem want to find the giraffe that is taller than the giraffe on their left and right.
```
...
```text
receive the number of giraffe then loop check and count if the previous and after value are fewer than that value 
```
